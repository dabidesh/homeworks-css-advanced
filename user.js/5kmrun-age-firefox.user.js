...
// @version      0.0.1
// @description  Show position by age achievement onclick (firefox bug)!
// @author       dabidesh
  ...
  window.onclick = () => {
    ...
    if (table) {
      ...
      for (let i = 1; i < rows.length; i++) {
        ...
        if (cell) {
          colAchievement.push(cell.querySelector('div').innerText);
        }
        let cellP = rows[i].cells[0];
        if (cellP) {
          colPositions.push(cellP.querySelector('div').innerText);
        }
      }
    }
    ...
    if (table) {
      ...
      for (let i = 1; i < rows.length; i++) {
        let cellP = rows[i].cells[0];
        if (cellP) {
          cellP.querySelector('div').innerText = colPositions[i - 1];
        }
      }
    }
  };
